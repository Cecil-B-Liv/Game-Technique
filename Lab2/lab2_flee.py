...
# =====================================================================
# Goal:
#   Show how an agent (triangle) can smoothly move away from a threat
#   using simple steering physics.
#
# Concepts reviewed:
#   - position, velocity, acceleration
#   - desired velocity pointing away from a threat
#   - steering = desired - velocity
#   - apply steering gradually to move away from the threat
# =====================================================================
...
pygame.display.set_caption("Steering Behavior: FLEE")
...
class Agent:
    ...
    def flee(self, threat):
        """Steer smoothly away from the threat."""
        desired = vec_sub(self.position, threat)
...
# -----------------------------
def main():
    """Run the FLEE behavior demo."""
    running = True
    agent = Agent(WIDTH // 2, HEIGHT // 2)
    threat = (WIDTH // 2, HEIGHT // 2)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                threat = pygame.mouse.get_pos()

        agent.flee(threat)
        agent.update()

        screen.fill((30, 30, 30))
        pygame.draw.circle(screen, (255, 60, 60), (int(threat[0]), int(threat[1])), 8)
        agent.draw(screen)

        font = pygame.font.SysFont("consolas", 20)
        text = font.render("Click anywhere - agent flees from the red threat", True, (255,255,255))
